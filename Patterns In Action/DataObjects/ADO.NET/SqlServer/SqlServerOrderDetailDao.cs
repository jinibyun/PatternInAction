...
{
    /// <summary>
    /// Sql Server specific data access object that handles data access of order details.
    /// </summary>
    public class SqlServerOrderDetailDao : IOrderDetailDao
    {
        /// <summary>
        ...
        {
            string sql =
            @"SELECT OrderId, O.ProductId, ProductName, O.UnitPrice, Quantity, Discount, O.Version
        ...
        private object[] Take(OrderDetail orderDetail)
        {
            return new object[]  
            {
			    ...
			    "@Version", orderDetail.Version.AsByteArray()
            };
        }
    }
}
